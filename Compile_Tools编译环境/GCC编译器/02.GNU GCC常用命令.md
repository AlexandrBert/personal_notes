...
## 2. STM32CubeMX 生成的 Makefile 运行打印分析
...
由以上文件可以看出，makefile 将：
1. 所有项目中包含的 `.c` 文件都用同样的命令编译成 `.o` 文件到新建的 build 目录下；
2. 启动文件 `.S` 文件编译成 `.o` 文件到新建的 build 目录下，不同于 C 文件的是该语法使用了 -x 命令显示的指定了文件的语言为汇编语言；
...
在 GCC 编译选项中，`-Wl` 用于向链接器（通常是 `ld`）传递参数。以下是对 `-Wl,-Map=build/led.map,--cref -Wl,--gc-sections` 的详细解释：

---

### **1. `-Wl` 的作用**
`-Wl` 是 GCC 的选项，用于将后续参数传递给链接器（`ld`）。  
- 格式：`-Wl,<ld-option1>,<ld-option2>,...`  
- 例如：`-Wl,-Map=output.map` 等价于 `ld -Map=output.map`。

---

### **2. `-Map=build/led.map`**
- **作用**：生成链接器生成的 **内存映射文件**（Memory Map File），记录程序的内存布局、符号地址、段大小等信息。  
- **用途**：调试时分析代码占用空间、优化内存布局。  
- **示例**：  
  ```sh
  gcc -Wl,-Map=build/led.map ...
  ```
  生成的文件 `led.map` 会包含：
  ```
...
### **3. `--cref`**
- **作用**：在映射文件中添加 **交叉引用表**（Cross Reference Table），显示符号在哪些目标文件/库中被引用。  
- **用途**：分析符号依赖关系，解决未定义符号错误。  
- **示例**（`led.map` 中会包含）：
  ```
...
### **4. `--gc-sections`**
- **作用**：启用 **垃圾回收（Garbage Collection）**，删除未被引用的代码段和数据段。  
- **原理**：链接器会遍历所有入口符号（如 `_start`、`main`），仅保留被直接或间接引用的段。  
- **用途**：显著减少固件体积（尤其适合嵌入式开发）。  
- **要求**：编译时需配合 `-ffunction-sections -fdata-sections`，让每个函数/变量单独成段。  
- **示例**：
  ```sh
  gcc -ffunction-sections -fdata-sections -Wl,--gc-sections ...
  ```

4. 使用 `arm-none-eabi-size` 命令打印出 `.elf` 文件的大小；
5. 使用 `arm-none-eabi-objcopy` 命令将 `.elf` 文件转换为 `.hex` 文件和 `.bin` 文件。