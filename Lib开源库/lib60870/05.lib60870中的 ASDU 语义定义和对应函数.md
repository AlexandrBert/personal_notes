...
## 2. 在监视方向的过程信息
### 2.1. 单点信息（M_SP_NA_1 = 1、M_SP_TB_1 = 30）
...
### 2.2. 双点信息（M_DP_NA_1 = 3、M_DP_TB_1 = 31）
...
### 2.3. 步进位置信息（M_ST_NA_1 = 5、M_ST_TB_1 = 32）
...
### 2.4. 32 比特串（M_BO_NA_1 = 7、M_BO_TB_1 = 33）
...
### 2.5. 不带品质描述符的归一化测量值（M_ME_ND_1 = 21）
...
### 2.6. 测量值，带品质描述符的归一化测量值（M_ME_NA_1 = 9、M_ME_TD_1 = 34）
...
### 2.7. 测量值，带品质描述符的标度测量值（M_ME_NB_1 = 11、M_ME_TE_1 = 35）
...
### 2.8. 测量值，带品质描述符的短浮点数（M_ME_NC_1 = 13、M_ME_TF_1 = 36）
...
### 2.9. 累计量（M_IT_NA_1 = 15、M_IT_TB_1 = 37）
...
### 2.10. 带时标的继电保护装置事件（M_EP_TD_1 = 38）
...
### 2.11. 带时标的继电保护装置成组启动事件（M_EP_TE_1 = 39）
...
### 2.12. 带时标的继电保护装置成组出口信息（M_EP_TF_1 = 40）
...
### 3.3. 升降命令（C_RC_NA_1 = 47）
`StepCommand` 用于控制步进电机或其他需要步进控制的设备。

步进命令通常用于电力系统中的断路器控制、调节设备（如变压器分接头调节器）等场景。
```c
/*******************************************
...
### 3.4. 设点命令，规一化值（C_SE_NA_1 = 48）
`SetpointCommandNormalized` 用于发送归一化的设定值命令，通常用于控制电力系统中的模拟量设备，如发电机的输出功率、变压器的分接头位置等。

归一化设定值通常是一个范围在 -1.0 到 1.0 之间的浮点数，表示设备的设定值相对于其最大和最小值的相对位置。
```c
/*************************************************
...
### 3.5. 设点命令，标度化值（C_SE_NB_1 = 49）
`SetpointCommandScaled` 用于发送缩放的设定值命令，通常用于控制电力系统中的模拟量设备，如发电机的输出功率、变压器的分接头位置等。

缩放设定值通常是一个整数，表示设备的设定值，其范围和单位由具体的应用场景定义。
```c
/*************************************************
...
### 3.6. 设点命令，短浮点数（C_SE_NC_1 = 50）
`SetpointCommandShort` 用于发送一个短浮点数设定值命令。这种命令通常用于控制那些需要高精度但数据范围相对较小的设备
```c
/*************************************************
...
### 3.7. 32 比特串（C_BO_NA_1 = 51）
`sBitstring32Command` 用于发送一个 32 位的位字符串命令。

这种命令通常用于表示多个二进制状态或配置信息，每个位可以表示一个独立的状态或控制信号。
```c
/*************************************************
...
## 4. 在控制方向的系统信息和参数
### 4.1. 总召命令（C_IC_NA_1 = 100）
`sInterrogationCommand` 在 lib60870 中可以用于表示总召命令（General Interrogation Command）。

总召命令通常用于请求从站提供所有当前的数据或状态信息。在 IEC 60870-5-101 和 IEC 60870-5-104 标准中，总召命令是一种常见的询问命令，用于获取从站的全部数据。
```c
/*************************************************
...
### 4.2. 读命令（C_RD_NA_1 = 102）
`sReadCommand` 用于发送读取命令，请求从站提供特定地址的数据点的当前值。

这种命令通常用于数据采集和监控系统中，帮助主站获取从站的特定数据点的状态或测量值。
```c
/*************************************************
...
### 4.3. 时钟同步命令（C_CS_NA_1 = 103）
`sClockSynchronizationCommand` 用于发送时钟同步命令，将主站的当前时间同步到从站。

这种命令通常用于时间同步系统中，帮助确保主站和从站之间的时间一致，这对于一些需要精确时间同步的应用非常重要。
...
### 4.4. 测量值参数，规一化值（P_ME_NA_1 = 110）
`sParameterNormalizedValue` 用于表示测量值参数，规一化值。这种类型的数据通常用于表示一些连续变化的物理量，如温度、压力等。
...
### 4.5. 测量值参数，标度化值（P_ME_NB_1 = 111）
适用于需要保持精度和节省带宽的场景，范围为 –32.768 到 32.767 ，用两个 8 位整数表示。
```c
/******************************************************
...
### 4.6. 测量值参数，短浮点数（P_ME_NC_1 = 112）
适用于需要高精度和通用性的场景，使用 32 位浮点数表示。
```c
/******************************************************
...
### 4.9. 测试命令（C_TS_NA_1 = 104、C_TS_TA_1 = 107）
